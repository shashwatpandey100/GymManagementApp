...
const page = () => {
    ...
    const fetchManagers = async () => {
      try {
        const res = await axios.get(
          `/api/v1/admin/managers?page=${currentPage}&pageSize=${pageSize}`
        );
        if (res?.data?.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.get(
        `/api/v1/admin/managers/search?search=${search}&page=${page}&pageSize=${pageSize}`
      );
      if (res.data.success) {
        setManagers(res.data.data.managers);
        setTotalPages(res.data.data.totalPages);
